...
# Setup LangSmith
setup_langsmith(project_name="multi-turn", enable_tracing=True)
...
with st.sidebar:
    ...
    # store 내용 상세 출력
    st.write("### Store 상세 내용:")
    for session_id, history in st.session_state["store"].items():
        st.write(f"세션 ID: {session_id}")
        st.write("메시지 기록:")
        for msg in history.messages:
            # type을 사용하여 메시지 종류 확인
            msg_type = msg.type
            st.write(f"- {msg_type}: {msg.content[:100]}...")  # 메시지 내용은 처음 100자만 표시
        st.write("---")
